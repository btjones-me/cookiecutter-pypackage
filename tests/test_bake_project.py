...
def test_bake_badge_in_readme_output_if_open_source(cookies):
    with bake_in_temp_dir(cookies, extra_context={"github_username": "UKHO", "open_source_license": "MIT license"}) as result:
        _, slug, _ = project_info(result)

        badge = f"![Python Package](https://github.com/UKHO/{slug}/workflows/Python%20package/badge.svg)"
...
def test_bake_badge_not_in_readme_output_if_open_source(cookies):
    with bake_in_temp_dir(cookies, extra_context={"github_username": "UKHO", "open_source_license": "Not open source"}) as result:
        _, slug, _ = project_info(result)
